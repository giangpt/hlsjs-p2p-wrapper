...
class HlsjsP2PWrapperPrivate {
    ...
    /**
     * Pass media elemennt to peer-agent, either synchronously if media element is already attached to hls.js, or when we receive hls.js's MEDIA_ATTACHING event
    ...
    createPeerAgent(p2pConfig, hlsjs, hlsEventsEnum, url = null) {
        ...
        let xhrSetup = noop;
        if (hlsjs.config && hlsjs.config.xhrSetup) {
            xhrSetup = hlsjs.config.xhrSetup;
        }

        this.peerAgentModule = new StreamrootPeerAgentModule(playerBridge, contentUrl, mediaMap, p2pConfig, SegmentView, streamType, integrationVersion, xhrSetup);
